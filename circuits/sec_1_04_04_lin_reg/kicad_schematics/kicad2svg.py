...
SVG_FILENAME = Path(".") / f"{SCH_NAME}.svg"


class Kicad:
    """KiCad netlist export command"""

    def __init__(self, kicad_cmd: Path, sch_filename: Path, svg_filename: Path) -> None:
        self.kicad_cmd: Path = kicad_cmd
        self.sch_filename: Path = sch_filename
        self.svg_filename: Path = svg_filename
...
def main() -> None:
    ...
    my_kicadcmd = Kicad(KICAD_CMD, SCH_FILENAME, SVG_FILENAME)
