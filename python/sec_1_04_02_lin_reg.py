...
def netlists_from_files(
    ...
    netlists_dict[Key.LOAD] = spi.Netlist(netlist_path / "load.cir")
    netlists_dict[Key.STIMULUS] = spi.Netlist(netlist_path / "stimulus.cir")
...
def create_dc_transfer_plot(
    dc1_results: spi.SimResults,
    vectors_dict: dict[str, spi.Vectors],
    results_path: Path,
) -> None:

    pwr1: spi.SimResults = copy.deepcopy(dc1_results)
    ...
    pwr2.vec_subset(vectors_dict[Key.VEC_ETA].list_out())

    # plot the efficiency vs. Vin
    plot_data = pwr2.x_axis_and_sigs(vectors_dict[Key.VEC_ETA].list_out())
...
def analyze_results(
    ...
    create_dc_transfer_plot(dc1_results, vectors_dict, paths_dict[Key.RESULTS_PATH])
...
def main() -> None:
    ...
    analyze_results(paths_dict, sim_results, vectors_dict)
